...
use_previous = 0 #Whether to use old other data and prices etc. 1 marks to use
emus = 10
...
SimAggr.get_control()
SimAggr.get_constraints(start,end)
SimAggr.get_other_input(start,end)
SimAggr.get_params()
...
for bldg in bldg_list:
	...
	for key in bldg_control.data:
		SimAggr.control.data[key+'_'+bldg] = variables.Timeseries(
			name = key+'_'+bldg,
			timeseries = pd.Series(np.random.rand(len(index))*0,index=index),
