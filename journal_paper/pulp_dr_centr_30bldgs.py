...
for simtime in sim_range:
    ...
    if simtime == DR_call_start:
        ...
        # Calculate aggregated reference profile
        j = 0
        for Sim in Sim_list:
            if j == 0:
                load_profiles = pd.Series(Sim.opt_controlseq['HPPower'].display_data().values, index = Sim.opt_controlseq['HPPower'].display_data().index) * Sim.compr_capacity
            else:
                load_profile = pd.Series(Sim.opt_controlseq['HPPower'].display_data().values, index = Sim.opt_controlseq['HPPower'].display_data().index) * Sim.compr_capacity
        ...
        for Sim in Sim_list:
            ...
            Sim.ref_profile.data['ref_profile'] = variables.Timeseries(
                            'ref_profile', 
                            load_profile_aggr / tot_cap, 
                            Sim.opt_controlseq['HPPower'].get_display_unit(), 
                            tz_name = Sim.weather.tz_name
                            )
        
        store_namespace('ref_profile_AggrMPC', Sim_list[0].ref_profile)
        store_namespace('flex_cost_AggrMPC', Sim_list[0].flex_cost)
    ...
    print("%%%%%% --- Optimising - Centralised --- %%%%%%")
        
    prob = None
    prob, hp_power, intemp, slack_1, slack_2,  dr_slack = lp_flexv1_centr(Sim_list, opt_index, simtime, DR_call_start, DRstart, DRend, rho, load_profile_aggr)
    ...
    for time in opt_index:
        ...
        for Sim in Sim_list:
            bldg  = Sim.building
            prob_out[time][bldg] = {
                'Power': hp_power[time][bldg].varValue,
                'InTemp': intemp[time][bldg].varValue,
                'Slack_1': slack_1[time][bldg].varValue,
                'Slack_2': slack_2[time][bldg].varValue,
                'dr_slack': dr_slack[time][bldg].varValue
            }
    
    result_df = pd.DataFrame.from_dict({(i,j): prob_out[i][j] 
                                            for i in prob_out.keys()
                                            for j in prob_out[i].keys()}, 
                                            orient='index'
                                            )
    #print(result_df['Power'][opt_index[0], Sim_list[0].building])
    #print(result_df['Power'])
    #result_df = result_df.shift(periods=1)
    #result_df = result_df.fillna(method='ffill')
    
    # Emulate response to the centralised solution
    for Sim in Sim_list:
        #print(result_df['Power'])
        #result_df['Power'][opt_index[0], Sim.building] = result_df['Power'][opt_index[1], Sim.building]
        #print(result_df['Power'])       
        result_power = pd.Series([result_df['Power'][time, Sim.building] for time in opt_index], index=opt_index) / (Sim.compr_capacity / 1000.0)
