...
'''
def simulate_mpc_JModelica(moinfo, emulation, parameters, weather, control,  other_input, timezone, start_time, end_time):
	...
	print("%%%%%%%%%%%%%%%----Starting Simulation of MPC---%%%%%%%%%%%%%")
	return	model, model.simulate(start_time, end_time)

def simulate_mpc_UKF(moinfo, emulation, parameters, weather, control,  other_input, timezone, start_time, end_time):
...
def store_namespace(filename,class_name):
	...
	#print("Namespace of class " + str(class_name) + " stored to " + str(filename))
	
def load_namespace(filename):
	with open(str(filename+'.pkl'), 'rb') as file:
		a = dill.load(file)
	#print("Namespace of class loaded from " + str(filename))
	return a
