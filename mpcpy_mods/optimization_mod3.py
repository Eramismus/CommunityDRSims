...
class Optimization(utility._mpcpyPandas, utility._Measurements):
    ...
    def __init__(self, Model, problem_type, package_type, objective_variable, **kwargs):    
        ...
        if 'add_objective' in kwargs:
            self.add_objective = kwargs['add_objective'];
...
class EnergyMin(_Problem):
    ...
    def _setup_jmodelica(self, JModelica, Optimization):
        ...
        JModelica.objective = 'mpc_model.' + Optimization.objective_variable+ 'rho*' + Optimization.add_objective;
        JModelica.extra_inputs = {};
        JModelica.extra_inputs['rho'] = [];
        JModelica.extra_inputs[Optimization.add_objective] = [];
...
class EnergyCostMin(_Problem):
    ...
    def _setup_jmodelica(self, JModelica, Optimization):
        ...
        JModelica.Model = Optimization.Model;
        JModelica.objective = 'mpc_model.' + Optimization.objective_variable + '*pi_e + rho*' + Optimization.add_objective;
        JModelica.extra_inputs = {};
...
class DRCostMin(_Problem):
    ...
    def _optimize(self, Optimization, **kwargs):
        ...
        #price_data = kwargs['price_data'];
        #ref_profile = kwargs['ref_profile'];
        #flex_cost = kwargs['flex_cost'];
        Optimization._package_type._DRcostmin(Optimization, **kwargs);
    ...
    def _setup_jmodelica(self, JModelica, Optimization):
        ...
        JModelica.Model = Optimization.Model;
        i = 0
        for item in Optimization.add_objective:
            if i == 0:
                add_obj = '(' + item
            elif i == len(Optimization.add_objective):
                add_obj = '+' + add_obj + ')'
            else:
                add_obj = add_obj + '+' + item
            i = i+1
        JModelica.objective = 'mpc_model.' + Optimization.objective_variable + '*pi_e + ' + '(mpc_model.' + Optimization.objective_variable + ' - ref_profile)*flex_cost + rho*' + add_obj + ')';
        JModelica.extra_inputs = {};
        JModelica.extra_inputs['pi_e'] = [];
        JModelica.extra_inputs['ref_profile'] = [];
        JModelica.extra_inputs['flex_cost'] = [];
        JModelica.extra_inputs['rho'] = [];

        for item in Optimization.add_objective:
            JModelica.extra_inputs[item] = [];
...
class JModelica(_Package, utility._FMU):
    ...
    def _energymin(self, Optimization):
        ...
        '''
        add_obj = kwargs['addobj']
        rho = kwargs['rho'];
        self.other_inputs['rho'] = rho['rho'];
        for item in Optimization.add_objective:
            self.other_inputs[item] = add_obj[item];
    ...
    def _DRcostmin(self, Optimization, **kwargs):
        '''Perform the energy cost minimization.
        ...
        add_obj = kwargs['addobj'];
        self.other_inputs['pi_e'] = price_data['pi_e'];
        self.other_inputs['ref_profile'] = ref_profile['ref_profile'];
        self.other_inputs['flex_cost'] = flex_cost['flex_cost'];        
        self.other_inputs['rho'] = rho['rho'];
        for item in Optimization.add_objective:
            self.other_inputs[item] = add_obj[item];
        self._simulate_initial(Optimization);
    ...
    def _initalize_mop(self):
        ...
        for key in self.Model.input_names:
            if key != 'pi_e' and key != 'flex_cost' and key != 'ref_profile' and key != 'rho' and key != 'Slack' and ('Slack' in key) == False:
        ...
        for key in Optimization.constraint_data.keys():
            for field in Optimization.constraint_data[key]:
                ...
                if field == 'GTE':
                    ...
                    self.mopfile.write('    der(mpc_model.' + key + ') <= ' + key_new + ';\n');
                elif field == 'Slack_GTE':
                    self.mopfile.write('    mpc_model.' + key + ' + ' + bldg + '_Slack >= ' + key_new + ';\n');
                elif field == 'Slack_LTE':
                    self.mopfile.write('    mpc_model.' + key + ' - ' + bldg + '_Slack <= ' + key_new + ';\n');   					
                elif field == 'Initial':
