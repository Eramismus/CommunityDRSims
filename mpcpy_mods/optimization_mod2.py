...
class DRCostMin(_Problem):
    ...
    def _setup_jmodelica(self, JModelica, Optimization):
        ...
        JModelica.Model = Optimization.Model;
        JModelica.objective = 'mpc_model.' + Optimization.objective_variable + '*pi_e + ' + '(mpc_model.' + Optimization.objective_variable + ' - ref_profile)*flex_cost + rho*' + Optimization.add_objective;
        JModelica.extra_inputs = {};
        JModelica.extra_inputs['pi_e'] = [];
...
class JModelica(_Package, utility._FMU):
    ...
    def _energycostmin(self, Optimization, **kwargs):
        ...
        self.other_inputs[Optimization.add_objective] = add_obj[Optimization.add_objective];
        self._simulate_initial(Optimization);
        self._solve(Optimization);   
        self._get_control_results(Optimization);
    ...
    def _initalize_mop(self):
        ...
        for key in self.Model.input_names:
            if key != 'pi_e' and key != 'flex_cost' and key != 'ref_profile' and key != 'rho' and key != 'Slack':
        ...
        for key in Optimization.constraint_data.keys():
            for field in Optimization.constraint_data[key]:
                ...
                if field == 'GTE':
                    ...
                    self.mopfile.write('    mpc_model.' + key + ' + Slack >= ' + key_new + ';\n');
