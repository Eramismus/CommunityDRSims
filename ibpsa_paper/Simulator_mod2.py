...
from mpcpy import optimization_mod2 as optimization
...
class SimHandler():
    def __init__(self, sim_start, sim_end, meas_sampl):
        ...
        self.addobj_varmap = {'Slack': ('Slack',units.unit1)
            }
        ...
        self.meas_vars_ref['PowerCompr']['Sample'] = variables.Static('sample_rate_Tzone',
                    self.meas_sampl, # sample rate
                    units.s
                    ) # units
    ...
    def get_constraints(self,start,end, **kwargs):  
        ...
        for i in index:
            if i.hour >= mor_start and i.hour <= mor_end:
                ...
                control_signal1[i] = np.random.uniform(0.0,0.0)
    ...
    def get_DRinfo(self,start,end,**kwargs):
        ...
        self.addobj.data = {"Slack": variables.Timeseries('Slack', pd.Series(0,index=index), units.unit1,tz_name=self.weather.tz_name)
        }
    ...
    def opt_control_minDRCost(self):
        ...
        #self.opt_options['blocking_factors'] = [1]*int(3*3600/self.meas_sampl)
        self.opt_problem.set_optimization_options(self.opt_options)
        self.opt_problem.optimize(self.opt_start,self.opt_end,
                                    price_data = self.price.data,
    ...
    def init_refmodel(self, use_fmu, use_const, const_path):
        ...
        if use_const == 1:
            t_set = load_namespace(os.path.join(const_path, 'constraints_'+self.building)).data['TAir']['Slack_GTE'].get_base_data()+1
        else:
    ...
    def run_reference(self, start, end):
        ...
        store_namespace('ref_heatinput_'+self.building, self.emuref.display_measurements('Measured')['PowerCompr'])
