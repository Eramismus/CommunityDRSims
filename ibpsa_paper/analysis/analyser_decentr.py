...
print(bldg_list)
folder = '10bldgs_decentr_dyn_fin_nov'
step = 300
nodynprice=0
mon = 'nov'
...
for bldg in bldg_list:
    ...
    flex_cost[building] = flex_cost[building].resample(str(step)+'S').mean()
    
    ref_profile[building] = load_namespace(os.path.join("path to sims", folder,'ref_profile_'+bldg+'_'+model_id)).data['ref_profile'].get_base_data()*3000
...
for bldg in bldg_list:
    ...
    for time in emutemps_df[bldg+'_'+model_id].index:
        ...
        #print(emutemp)
        constraint_hi = constraints_df.loc[bldg]['hi'][time]
        constraint_lo = constraints_df.loc[bldg]['lo'][time]
        #print(time)
        #print(constraint_hi)
        #print(constraint_lo)
...
for bldg in [bldg_list[0], bldg_list[4],bldg_list[6],bldg_list[9]]:
